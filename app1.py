...
from difflib import SequenceMatcher # LIBRARY BARU UNTUK MENGHITUNG % KEMIRIPAN
...
def calculate_metrics(reference, hypothesis):
    """
    Menghitung BLEU Score dan Similarity dengan perbaikan batas nilai (Clamping).
    """
    # 1. Normalisasi
    ref_clean = str(reference).lower().strip()
    hyp_clean = str(hypothesis).lower().strip()
    
    # 2. Hapus tanda baca
    ref_clean = re.sub(r"[^a-zA-Z0-9\s]", "", ref_clean)
    hyp_clean = re.sub(r"[^a-zA-Z0-9\s]", "", hyp_clean)
    
    # 3. Tokenisasi
    reference_tokens = [ref_clean.split()] # List of lists
    hypothesis_tokens = hyp_clean.split()  # List of tokens
    ...
    # --- PERBAIKAN: Memastikan skor tidak lebih dari 1.0 ---
    if bleu_score > 1.0:
        bleu_score = 1.0
    
    # 5. Similarity (Sequence Matcher)
    matcher = SequenceMatcher(None, ref_clean, hyp_clean)
    similarity = matcher.ratio() * 100 # Persen
    
    return {"bleu": bleu_score, "accuracy": similarity}
...
if 'reference_translation' not in st.session_state:
    st.session_state.reference_translation = ""
    ...
    if not input_text.strip():
        ...
        with st.spinner("Translating... (Encoder → Attention → Decoder)"):
            try:
                ...
                processed_input_for_lookup = preprocess_english(input_text)
                reference = dataset_lookup.get(processed_input_for_lookup)

                if reference:
                    st.session_state.reference_translation = reference
                    metrics = calculate_metrics(reference, result_dict.get('translation', ''))
